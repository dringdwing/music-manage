...
<script>
import {allSong, getAllConsumer, getAllSinger, getAllSongList} from '../api'
  ...
  methods: {
    getConsumer () {
      getAllConsumer().then(res => {
        ...
        this.consumerSex.rows[0]['总数'] = this.setSex(1, this.consumer) // 1：♂
        this.consumerSex.rows[1]['总数'] = this.setSex(0, this.consumer) // 1：♀
      })
    },
    ...
    getSinger () { // 歌手数量
      getAllSinger().then(res => {
        ...
        this.singerSex.rows[0]['总数'] = this.setSex(0, res) // 1：女
        this.singerSex.rows[1]['总数'] = this.setSex(1, res) // 1：男
        this.singerSex.rows[2]['总数'] = this.setSex(2, res) // 1：组合
        this.singerSex.rows[3]['总数'] = this.setSex(3, res) // 1：不明
        for (let item of res) {
